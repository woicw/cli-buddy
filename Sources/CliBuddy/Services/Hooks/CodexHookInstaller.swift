...
//  CliBuddy
//
//  Installs/uninstalls cli-buddy hooks into Codex's hooks.json and config.toml.
...
struct CodexHookInstallerManifest: Equatable, Codable, Sendable {
    static let fileName = "cli-buddy-codex-install.json"
    static let legacyFileName = "code-island-codex-install.json"
...
enum CodexHookInstaller {
    static let managedStatusMessage = "Managed by cli-buddy"
    static let legacyManagedStatusMessage = "Managed by Code Island"
    ...
    static func installIfNeeded() {
        ...
        let scriptPath = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".claude/hooks/cli-buddy-state.py").path
        ...
        for (_, value) in hooksObj {
            ...
            for group in groups {
                if let hooks = group["hooks"] as? [[String: Any]] {
                    for hook in hooks {
                        if let cmd = hook["command"] as? String, cmd.contains("cli-buddy-state.py") {
                            return true
                        }
                    }
                }
